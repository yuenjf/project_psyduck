...
<title>${categoryp.categoryName}_可达鸭美食指南</title>
...
$(function() {
	 ...
	 * 分类功能
	 	第一：点击分类类别，该类别高亮
	 	第二：判断拿到的List是否为空，如果为空，显示noResult内容,隐藏分页按钮
	 	第三：判断当前页，如果为第一页，则不能按上一页，若果为最后一页，则不能按下一页
	 */
	$(".list-group>a").eq(${categoryp.item-1}).addClass("active");
	
	if(${categoryp.categoryPageDetail==null}) {
		$(".noResult").removeClass("hidden");
		$(".page").addClass("hidden");
	}
	
	if(${categoryp.currentPage==1}){
		$(".page>ul>li:eq(0)").addClass("disabled");
		$(".page>ul>li:eq(0)>a").attr("href","javascript:;");
	}
	if(${categoryp.currentPage}==${categoryp.totlePage}){
...
<body>
	...
	<div class="mian">
		<div class="container clearfix">
			...
			<div class="col-md-7">
				<div class="rec-list">
					<h2>${categoryp.categoryName}</h2>
					<ul>
						<c:forEach items="${categoryp.categoryPageDetail}" var="i">
							<li class="clearfix">
					...
					<h3 class="noResult hidden">此分类还有有菜谱，赶紧去创建一个吧！</h3>
					<!-- 分页按钮 -->
					<div class="page">
						<ul class="pager">
							<li><a href="recipeCategory?item=${categoryp.item}&categoryName=${categoryp.categoryName}&categoryCurrentPage=${categoryp.currentPage-1}">上一页</a></li>
							<li><a href="recipeCategory?item=${categoryp.item}&categoryName=${categoryp.categoryName}&categoryCurrentPage=${categoryp.currentPage+1}">下一页</a></li>
							<li class="page-info">第${categoryp.currentPage}/${categoryp.totlePage}页</li>
						</ul>
					</div>
				</div>
			</div>
